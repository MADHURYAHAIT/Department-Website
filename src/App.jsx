...
function App() {
  ...
  useEffect(() => {
    ...
    if (loadTime > 0) {
      ...
      // If the load time is 0, add an event listener to wait for the load event
      const handleLoad = () => {
        setPageLoaded(true);
      };
    ...
    document
      ...
      .forEach((link) => link.addEventListener("click", handleMenuLinkClick));
    document
    ...
    return () => {
      ...
      document
        .querySelectorAll(".main-header-link")
        .forEach((link) =>
          link.removeEventListener("click", handleMainHeaderLinkClick)
        );
      document
        .querySelectorAll(".dropdown")
        .forEach((dropdown) =>
      ...
      document
        .querySelector(".search-bar input")
        .removeEventListener("focus", handleSearchBarFocus);
      document
        .querySelector(".search-bar input")
        .removeEventListener("blur", handleSearchBarBlur);
      document.removeEventListener("click", handleDocumentClick);
      document
        .querySelectorAll(".dropdown")
        .forEach((dropdown) =>
          dropdown.removeEventListener("click", handleDropdownClickOverlay)
        );
      ...
      document
        .querySelector(".dark-light")
        .removeEventListener("click", handleToggleButtonClick);
    };
  }, []);
  ...
  return (
    <>
      {!isPageLoaded && <PreLoader />} {/* Show the loader until the page is loaded */}
      {isPageLoaded && (
      <div className="App">
